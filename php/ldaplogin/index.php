...
function ldap_connection()
{
    ...
    try {
        ...
        try {
            if ($bind) {
                ldap_close($ldap_con);
                print '<code class="green">Login successful!</code>';
            } else {
                print '<code class="red">LDAP Error: ' . ldap_error($ldap_con) . '</code>';
            }
        } catch (Exception $e) {
            print '<code class="red">LDAP Error: ' . ldap_error($ldap_con) . "/" . $e->getMessage() . '</code>';
        }
    } catch (Error $e) {
        print '<code class="red">Error: Something went completely wrong! - ' . $e->getMessage() . '</code>';
    }
}
?>
